...
def main(exp_args, exp_name):
    ...
    with open(join(save_dir, "success_rate_" + str(exp_args.planner_type) + "_exp-" + str(exp_args.data_dir.split("/")[-1]) + "_hact-" + str(exp_args.human_act_as_cond) + ".txt"), "a") as f:
            ...
            for k in range(len(goal_cfgs)):
                ...
                for j in range(len(obs_cfgs)):
                    ...
                    if not (exp_args.human_mode == "data" and exp_args.robot_mode == "data"):
                        ...
                        print("Success counter: {0}, currently on {1} trial of {2}.".format(sub_test_success_rate, str(j), len(obs_cfgs)))
                        f.write("\t\tObs_cfg {0}: Success:{1}.\n".format(obs_cfgs[j], success))

                f.write("\t\tObs_cfg sub_test success rate: {}.\n".format(obs_sub_test_success_rate / len(obs_cfgs)))
                obs_sub_test_success_rate = 0


            print("Sub-test: table_cfg {0} goal_cfg {1}. Sub success rate (IV = table cfg): {2} (of {3} trials).\n".format(table_cfgs[i], goal_cfgs[k], sub_test_success_rate, len(goal_cfgs) * len(obs_cfgs)))
            
            f.write("\tSub-test: table_cfg {0} goal_cfg {1}. Sub success rate w/ varying table cfg: {2} of {3} trials = {4}.\n".format(table_cfgs[i], goal_cfgs[k], sub_test_success_rate, len(goal_cfgs) * len(obs_cfgs), sub_test_success_rate / (len(goal_cfgs) * len(obs_cfgs))))
            sub_test_success_rate = 0
        ...
        success_rate /= (len(table_cfgs) * len(goal_cfgs) * len(obs_cfgs))
        success_duration /= num_success if num_success > 0 else 1
        avg_plan_time_total /= (len(table_cfgs) * len(goal_cfgs) * len(obs_cfgs))
        print("Success rate: {0}, with {1} total trials. Avg duration of {2} for {3} total successes. Avg plan time {4}.".format(success_rate, len(table_cfgs) * len(goal_cfgs) * len(obs_cfgs), success_duration, num_success, avg_plan_time_total))

        f.write("{0}    Subject ID: {1}. Success rate: {2}, with {3} total trials. Avg duration of {4} for {5} total successes. Avg plan time {6}.\n".format(time.strftime("%c", time.localtime()), exp_args.subject_id, success_rate, len(table_cfgs) * len(goal_cfgs) * len(obs_cfgs), success_duration, num_success, avg_plan_time_total))
...
if __name__ == "__main__":
    ...
    if exp_args.robot_mode == "planner":
        robot_str = exp_args.robot_mode + "-" + exp_args.planner_type + "-UNET"
    else:
        robot_str = exp_args.robot_mode
    human_str = exp_args.human_mode + "-" + exp_args.human_control
